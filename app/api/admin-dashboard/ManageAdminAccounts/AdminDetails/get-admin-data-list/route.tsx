...
export async function GET() {
  ...
  if (cookieStore.has("email")) {
    ...
    console.log(
      "admindashboard--->manage accounts--->admin email--> email :" +
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/get-all-admin-list?adminEmail=${emailValueString}`,
      {
        // default it is GET
        headers: {
    ...
    if (response.ok) {
      ...
      console.warn(
        response.status +
          " >> Error from admin details fetching >> " +
          responseBodyText
      );
