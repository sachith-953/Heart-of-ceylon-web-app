...
/**
 * this API use to add new admins from admin dashboard
...
export async function POST(request: Request) {
  ...
  console.log("\n add-new-admin > : Nextjs API has Called");
  ...
  console.log("add-new-admin >FORM RECIEVED TO THE NEXT JS ENDPOINT");
  ...
  try {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/add-admin?adminEmail=${emailValueString}`,
      {
        method: "POST",
        headers: {
    ...
    if (response.ok) {
      ...
      const resData = { message: responseBodyText };
      console.warn(
        response.status + " >> Error from add new admin >> " + responseBodyText
      );
