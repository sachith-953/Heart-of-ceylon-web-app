...
export async function POST(request: Request) {
  ...
  console.log("\nReject product : Nextjs API has Called");
  ...
  if (cookieStore.has("email")) {
    ...
    console.log(
      "AdminDashboard > product management -->Reject product-->> email :" +
  ...
  console.log(
    "AdminDashboard >Reject product > email >>>: " + emailValueString
  );
  ...
  try {
    const response = await fetch(
      `http://localhost:8080/api/v1/auth/reject-product-by-admin?adminEmail=${emailValueString}&productID=${productID}`,
      {
        method: "DELETE",
        headers: {
    ...
    console.log("Reject product --> request Sent to backend");
    if (response.ok) {
      ...
      console.log("Reject product -->successfully:", data);
