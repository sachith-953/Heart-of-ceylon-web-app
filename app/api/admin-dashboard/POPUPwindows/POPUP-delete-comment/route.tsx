...
export async function POST(request: Request) {
  ...
  console.log("\nDelete comment: Nextjs API has Called");
  ...
  if (cookieStore.has("email")) {
    ...
    console.log(
      "AdminDashboard > delete a comment for a product--> email :" +
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/delete-product-review-by-admin?adminEmail=${emailValueString}&reviewId=${reviewId}`,
      {
        method: "PUT",
        headers: {
    ...
    if (response.ok) {
      ...
      console.warn(
        response.status +
          " >> Error from---> delete a review for a product>> " +
          responseBodyText
      );
