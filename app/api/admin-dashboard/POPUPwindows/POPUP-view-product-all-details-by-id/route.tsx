...
export async function POST(request: Request) {
  ...
  if (cookieStore.has("email")) {
    ...
    console.log(
      "AdminDashboard > View product details--> popup> email :" +
  ...
  console.log(
    "AdminDashboard > View product all details by id-->popup > email >>>: " +
      emailValueString
  );
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/get-all-details-of-a-product?adminEmail=${emailValueString}&productID=${productID}`,
      {
        // default it is GET
        headers: {
    ...
    if (response.ok) {
      ...
      console.warn(
        response.status +
          " >> Error from---> getting product details by id >> " +
          responseBodyText
      );
