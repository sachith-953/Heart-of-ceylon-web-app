...
export async function POST(request: Request) {
  ...
  if (cookieStore.has("email")) {
    ...
    console.log(
      "AdminDashboard > product management -->Get reviews and ratings--> popup> email :" +
  ...
  console.log(
    "AdminDashboard > get reviews and ratings-->popup > email >>>: " +
      emailValueString
  );
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/get-reviews-for-a-product?adminEmail=${emailValueString}&productID=${productID}`,
      {
        // default it is GET
        headers: {
    ...
    console.log(
      "Get reviews and ratings for a product --> request Sent to backend"
    );
    if (response.ok) {
      ...
      console.warn(
        response.status +
          " >> Error from---> fetching reviews and ratings >> " +
          responseBodyText
      );
