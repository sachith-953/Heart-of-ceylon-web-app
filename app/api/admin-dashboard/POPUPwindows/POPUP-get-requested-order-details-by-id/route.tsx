...
export async function POST(request: Request) {
  ...
  if (cookieStore.has("email")) {
    ...
    console.log(
      "AdminDashboard > requested order details for a requested order--> popup> email :" +
  ...
  console.log(
    "AdminDashboard > View requested order details for a requested order ->popup > email >>>: " +
      emailValueString
  );
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/get-more-details-for-requested-order?adminEmail=${emailValueString}&requestedOrderID=${requestedOrderID}`,
      {
        // default it is GET
        headers: {
    ...
    console.log(
      "View get-more-details-for-requested-order --> request Sent to backend"
    );
    if (response.ok) {
      ...
      console.log(
        "View get-more-details-for-requested-order-->data fetched successfully:",
        data
      );
      ...
      console.warn(
        response.status +
          " >> Error from---> View get-more-details-for-requested-order >> " +
          responseBodyText
      );
