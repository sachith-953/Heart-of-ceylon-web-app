...
export async function POST(request: Request) {
  ...
  if (cookieStore.has("email")) {
    ...
    console.log(
      "AdminDashboard > remove product--> email :" + JSON.stringify(email)
    );
    //output :
  ...
  console.log(
    "AdminDashboard > remove a product > email >>>: " + emailValueString
  );
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/delete-product-by-admin?adminEmail=${emailValueString}&productID=${productID}`,
      {
        method: "DELETE",
        headers: {
    ...
    if (response.ok) {
      ...
      console.warn(
        response.status +
          " >> Error from---> remove a product by admin >> " +
          responseBodyText
      );
