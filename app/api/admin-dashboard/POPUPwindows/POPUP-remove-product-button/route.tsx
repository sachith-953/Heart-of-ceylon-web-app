...
export async function POST(request: Request) {
    ...
    console.log("\nRemove product: Nextjs API has Called");
    ...
    if (cookieStore.has('email')) {
        ...
        console.log("AdminDashboard > remove product--> email :" + JSON.stringify(email));
        //output : 
    ...
    console.log("AdminDashboard > remove a product > email >>>: " + emailValueString);
    ...
    console.log("remove  a product--> Nextjs API has Called");
    console.log("productID = " + productID)

    try{
        const response = await fetch(`http://localhost:8080/api/v1/auth/delete-product-by-admin?adminEmail=${emailValueString}&productID=${productID}`, {
           method : "DELETE",
            headers: {
        ...
        console.log("remove a product by admin--> request Sent to backend");
        if (response.ok) {
            const data = await response.text(); 
            // if the backend return a string this should response.text()
            console.log("remove a product by admin successfull:", data);
            ...
            console.warn(response.status + " >> Error from---> remove a product by admin >> " + responseBodyText)
            return new Response(
