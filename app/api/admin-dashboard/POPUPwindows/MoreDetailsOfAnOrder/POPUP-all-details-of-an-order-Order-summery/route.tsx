...
export async function POST(request: Request) {
    ...
    console.log("\nPOPUP-all-details-of-an-order-Order-summery: Nextjs API has Called");
    ...
    if (cookieStore.has('email')) {
        ...
        console.log("AdminDashboard > POPUP-all-details-of-an-order-Order-summery> email :" + JSON.stringify(email));
        //output : 
    ...
    console.log("AdminDashboard > POPUP-all-details-of-an-order-Order-summery> email >>>: " + emailValueString);
    ...
    console.log("POPUP-all-details-of-an-order-Order-summery--> Nextjs API has Called");

    try{
        const response = await fetch(`http://localhost:8080/api/v1/auth/get-order-summery-for-an-order?adminEmail=${emailValueString}&orderID=${orderID}`, {
           // default it is GET
            headers: {
        ...
        console.log("POPUP-all-details-of-an-order-Order-summery---> request Sent to backend");
        if (response.ok) {
            ...
            console.log("POPUP-all-details-of-an-order-Order-summery--> fetched successfully:", data);
            return new Response(
                JSON.stringify(data), 
                {
                status: 200,
            ...
            console.warn(response.status + " >> Error from---> POPUP-all-details-of-an-order-Order-summery >> " + responseBodyText)
            return new Response(
