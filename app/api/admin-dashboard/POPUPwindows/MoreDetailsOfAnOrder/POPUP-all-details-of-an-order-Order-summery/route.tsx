...
export async function POST(request: Request) {
  ...
  if (cookieStore.has("email")) {
    ...
    console.log(
      "AdminDashboard > POPUP-all-details-of-an-order-Order-summery> email :" +
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/get-order-summery-for-an-order?adminEmail=${emailValueString}&orderID=${orderID}`,
      {
        // default it is GET
        headers: {
    ...
    console.log(
      "POPUP-all-details-of-an-order-Order-summery---> request Sent to backend"
    );
    if (response.ok) {
      ...
      console.log(
        "POPUP-all-details-of-an-order-Order-summery--> fetched successfully:",
        data
      );
      ...
      console.warn(
        response.status +
          " >> Error from---> POPUP-all-details-of-an-order-Order-summery >> " +
          responseBodyText
      );
