...
export async function POST(request: Request) {
  ...
  if (cookieStore.has("email")) {
    ...
    console.log(
      "AdminDashboard > POPUP-all-details-of-an-order-Buyer-details> email :" +
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/get-buyer-details-for-order?adminEmail=${emailValueString}&orderID=${orderID}`,
      {
        // default it is GET
        headers: {
    ...
    console.log(
      "POPUP-all-details-of-an-order-Buyer-details--> request Sent to backend"
    );
    if (response.ok) {
      ...
      console.log(
        "POPUP-all-details-of-an-order-Buyer-details--> fetched successfully:",
        data
      );
      ...
      console.warn(
        response.status +
          " >> Error from---> POPUP-all-details-of-an-order-Buyer-details >> " +
          responseBodyText
      );
