...
export async function POST(request: Request) {
    ...
    console.log("\nPOPUP-all-details-of-an-order-Buyer-details: Nextjs API has Called");
    ...
    if (cookieStore.has('email')) {
        ...
        console.log("AdminDashboard > POPUP-all-details-of-an-order-Buyer-details> email :" + JSON.stringify(email));
        //output : 
    ...
    console.log("AdminDashboard > POPUP-all-details-of-an-order-Buyer-details> email >>>: " + emailValueString);
    ...
    console.log("POPUP-all-details-of-an-order-Buyer-details--> Nextjs API has Called");

    try{
        const response = await fetch(`http://localhost:8080/api/v1/auth/get-buyer-details-for-order?adminEmail=${emailValueString}&orderID=${orderID}`, {
           // default it is GET
            headers: {
        ...
        console.log("POPUP-all-details-of-an-order-Buyer-details--> request Sent to backend");
        if (response.ok) {
            ...
            console.log("POPUP-all-details-of-an-order-Buyer-details--> fetched successfully:", data);
            return new Response(
                JSON.stringify(data), 
                {
                status: 200,
            ...
            console.warn(response.status + " >> Error from---> POPUP-all-details-of-an-order-Buyer-details >> " + responseBodyText)
            return new Response(
