...
export async function POST(request: Request) {
    ...
    console.log("\nView buyer details for a requested order: Nextjs API has Called");
    ...
    if (cookieStore.has('email')) {
        ...
        console.log("AdminDashboard > View buyer details for a requested order--> popup> email :" + JSON.stringify(email));
        //output : 
    ...
    console.log("AdminDashboard > View buyer details for a requested order ->popup > email >>>: " + emailValueString);
    ...
    console.log("View buyer details for a requested order--> Nextjs API has Called");

    try{
        const response = await fetch(`http://localhost:8080/api/v1/auth/get-buyer-details-for-requested-order?adminEmail=${emailValueString}&requestedOrderID=${requestedOrderID}`, {
           // default it is GET
            headers: {
        ...
        console.log("View buyer details for a requested order--> request Sent to backend");
        if (response.ok) {
            ...
            console.log("View buyer details for a requested order-->data fetched successfully:", data);
            return new Response(
                JSON.stringify(data), 
                {
                status: 200,
            ...
            console.warn(response.status + " >> Error from---> View buyer details for a requested order >> " + responseBodyText)
            return new Response(
