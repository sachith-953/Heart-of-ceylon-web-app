...
export async function POST(request: Request) {
  ...
  if (cookieStore.has("email")) {
    ...
    console.log(
      "AdminDashboard > View buyer details for a requested order--> popup> email :" +
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/get-buyer-details-for-requested-order?adminEmail=${emailValueString}&requestedOrderID=${requestedOrderID}`,
      {
        // default it is GET
        headers: {
    ...
    console.log(
      "View buyer details for a requested order--> request Sent to backend"
    );
    if (response.ok) {
      ...
      console.log(
        "View buyer details for a requested order-->data fetched successfully:",
        data
      );
      ...
      console.warn(
        response.status +
          " >> Error from---> View buyer details for a requested order >> " +
          responseBodyText
      );
