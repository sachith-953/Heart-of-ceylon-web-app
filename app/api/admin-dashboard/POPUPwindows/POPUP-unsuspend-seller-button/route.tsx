...
export async function POST(request: Request) {
    ...
    console.log("\Unsuspend seller : Nextjs API has Called");
    ...
    if (cookieStore.has('email')) {
        ...
        console.log("AdminDashboard > seller management -->Unsuspend seller-->> email :" + JSON.stringify(email));
        //output : 
    ...
    console.log("AdminDashboard >Unsuspend seller > email >>>: " + emailValueString);
    ...
    try {
        const response = await fetch(`http://localhost:8080/api/v1/auth/unlock-seller-account?adminEmail=${emailValueString}&sellerID=${sellerID}`, {
            method: 'PUT',
            headers: {
        ...
        console.log("Unsuspend seller --> request Sent to backend");
        if (response.ok) {
            ...
            console.log("Unsuspend seller -->successfully:", data);
            return new Response(
                JSON.stringify(data),
                {
                    status: 200,
            ...
            console.warn(response.status + " >> Error from---> Unsuspend seller>> " + responseBodyText)
            return new Response(
