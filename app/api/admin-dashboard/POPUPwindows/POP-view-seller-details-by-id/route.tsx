...
export async function POST(request: Request) {
  ...
  if (cookieStore.has("email")) {
    ...
    console.log(
      "AdminDashboard > View seller details--> popup> email :" +
  ...
  console.log(
    "AdminDashboard > View seller details by id-->popup > email >>>: " +
      emailValueString
  );
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/get-sellers-all-data?adminEmail=${emailValueString}&sellerID=${sellerID}`,
      {
        // default it is GET
        headers: {
    ...
    if (response.ok) {
      ...
      console.warn(
        response.status +
          " >> Error from---> getting seller details by id >> " +
          responseBodyText
      );
