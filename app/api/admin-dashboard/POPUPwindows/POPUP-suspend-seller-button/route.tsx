...
export async function POST(request: Request) {
    ...
    console.log("\nSuspend seller : Nextjs API has Called");
    ...
    if (cookieStore.has('email')) {
        ...
        console.log("AdminDashboard > seller management -->Suspend seller-->> email :" + JSON.stringify(email));
        //output : 
    ...
    console.log("AdminDashboard >Suspend seller> email >>>: " + emailValueString);
    ...
    console.log("Suspend seller --> Nextjs API has Called");

    try {
        const response = await fetch(`http://localhost:8080/api/v1/auth/update-seller-status-by-admin?adminEmail=${emailValueString}&sellerID=${sellerID}`, {
            method: 'PUT',
            headers: {
            ...
            body: JSON.stringify({
                sellerStatus: "SUSPEND"
            }),
        });
        console.log("Suspend seller --> request Sent to backend");
        if (response.ok) {
            ...
            console.log("Suspend seller -->successfully:", data);
            return new Response(
                JSON.stringify(data),
                {
                    status: 200,
            ...
            console.warn(response.status + " >> Error from---> Suspend seller>> " + responseBodyText)
            return new Response(
