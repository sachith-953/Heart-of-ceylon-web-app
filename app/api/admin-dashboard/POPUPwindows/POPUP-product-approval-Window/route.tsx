...
export async function POST(request: Request) {
    ...
    console.log("\nGet Product details to verify: Nextjs API has Called");
    ...
    if (cookieStore.has('email')) {
        ...
        console.log("AdminDashboard > product management -->Get Product details to verify--> popup> email :" + JSON.stringify(email));
        //output : 
    ...
    console.log("AdminDashboard > View product to verify-->popup > email >>>: " + emailValueString);
    ...
    console.log("get-all details of a product for verify --> Nextjs API has Called");

    try{
        const response = await fetch(`http://localhost:8080/api/v1/auth/get-all-details-of-a-product-to-verify?adminEmail=${emailValueString}&productID=${productID}`, {
           // default it is GET
            headers: {
        ...
        console.log("View all details of a product for verification --> request Sent");
        if (response.ok) {
            ...
            console.log("get product all details for verify --> fetched successfully:", data);
            return new Response(
                JSON.stringify(data), 
                {
                status: 200,
            ...
            console.warn(response.status + " >> Error from---> getting product details for verify >> " + responseBodyText)
            return new Response(
