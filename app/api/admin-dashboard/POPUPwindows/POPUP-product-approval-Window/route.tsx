...
export async function POST(request: Request) {
  ...
  if (cookieStore.has("email")) {
    ...
    console.log(
      "AdminDashboard > product management -->Get Product details to verify--> popup> email :" +
  ...
  console.log(
    "AdminDashboard > View product to verify-->popup > email >>>: " +
      emailValueString
  );
  ...
  console.log(
    "get-all details of a product for verify --> Nextjs API has Called"
  );

  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/get-all-details-of-a-product-to-verify?adminEmail=${emailValueString}&productID=${productID}`,
      {
        // default it is GET
        headers: {
    ...
    console.log(
      "View all details of a product for verification --> request Sent"
    );
    if (response.ok) {
      ...
      console.log(
        "get product all details for verify --> fetched successfully:",
        data
      );
      ...
      console.warn(
        response.status +
          " >> Error from---> getting product details for verify >> " +
          responseBodyText
      );
