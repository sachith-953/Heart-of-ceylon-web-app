...
/**
 * this API use to search any sellers in admin dashboard
...
export async function POST(request: Request) {
    ...
    console.log("\n AdminDashboard > seller managment--> search an seller: Nextjs API has Called");
    ...
    if (cookieStore.has('email')) {
        ...
        console.log("AdminDashboard > seller managment--> search an seller: > email :" + JSON.stringify(email));
        //output : 
    ...
    console.log("AdminDashboard > seller managment--> search an seller:  > email >>>: " + emailValueString);
    ...
    console.log("AdminDashboard > seller managment--> search an seller:--> Nextjs API has Called");

    try{
        const response = await fetch(`${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/get-sellers-by-search?adminEmail=${emailValueString}&searchWord=${searchKey}&pageNumber=${requestedPageNo}`, {
           // default it is GET
            headers: {
        ...
        console.log("AdminDashboard > seller managment--> search an seller:--> request Sent");
