...
/**
 * this API use to get requested order deatails in admin dashboard
...
export async function GET() {
    ...
    console.log("\nAdminDashboard > Get requested orders for admin dashboard : Nextjs API has Called");
    ...
    if (cookieStore.has('email')) {
        ...
        console.log("All orders-->requeseted orders   > email :" + JSON.stringify(email));
        //output : 
    ...
    console.log("admindhaboard -->All orders-->requeseted orders  > email >>>: " + emailValueString);
    ...
    try{
        ...
        const response = await fetch(`http://localhost:8080/api/v1/auth/get-50-requested-order-details?adminEmail=${emailValueString}`, {
           // default it is GET
            headers: {
    ...
    console.log("get-50- requested order-details -----> request Sent");
    if (response.ok) {
        ...
        console.log("Admindashboard --> All orders--requested order--->requested order details fetched successfully:", data);
        return new Response(
            JSON.stringify(data), 
            {
            status: 200,
        ...
        console.warn(response.status + " >> Admindashboard --> All orders-- requested order--->order details fetching >> " + responseBodyText)
        return new Response(
