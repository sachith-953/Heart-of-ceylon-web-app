...
/**
 * this API use to update order details in admin dashboard
...
export async function POST(request: Request) {
    ...
    console.log(" update order details --> search an order: Nextjs API has Called");
    ...
    if (cookieStore.has('email')) {
        ...
        console.log("update order details -> search an order > email :" + JSON.stringify(email));
        //output : 
    ...
    console.log("update order details  > email >>>: " + emailValueString);
    ...
    const note = reqParams.productNote
    const orderId = reqParams.orderId
    const newDeliveryDate = reqParams.newDeliveryDate
    const newStatus = reqParams.newStatus

    console.log("OrderId : " + orderId + " > note : " + note)
    console.log("newDeliveryDate : " + newDeliveryDate + " > newStatus : " + newStatus)

    //if searchKey is not entered or empty spaces has entered, we return an error
    if (orderId === null || orderId === undefined) {
        // return a response for a error
        const resData = { success: false, message: " order Id is Empty !!" }
    ...
    try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/update-order-details-for-an-order?adminEmail=${emailValueString}&orderID=${orderId}&notes=${note}&expectedDeliveryDate=${newDeliveryDate}&orderStatus=${newStatus}`, {
            method: 'PUT',
            headers: {
        ...
        console.log("update order details--> request Sent");
