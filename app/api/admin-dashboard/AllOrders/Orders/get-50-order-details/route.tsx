...
export async function POST(request: Request) {
    ...
    console.log("\nAdminDashboard > Get orders for admin dashboard : Nextjs API has Called");
    ...
    if (cookieStore.has('email')) {
        ...
        console.log("All orders-->orders   > email :" + JSON.stringify(email));
        //output : 
    ...
    console.log("admindhaboard -->All orders-->orders  > email >>>: " + emailValueString);
    ...
    console.log("Admindashboard --> All orders-->orders. Nextjs API has Called");
    ...
    try{
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/get-50-order-details?adminEmail=${emailValueString}&pageNumber=${requestedPageNo}`, {
           // default it is GET
            headers: {
    ...
    console.log("get-50-order-details -----> request Sent");
    if (response.ok) {
        ...
        console.log("Admindashboard --> All orders--orders--->order details fetched successfully:", data);
        return new Response(
            JSON.stringify(data), 
            {
            status: 200,
        ...
        console.warn(response.status + " >> Admindashboard --> All orders--orders--->order details fetching >> " + responseBodyText)
        return new Response(
