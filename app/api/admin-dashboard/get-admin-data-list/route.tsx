...
/**
 * this API use to get admin details list for manageaccounts in admin dashboard
 ...
 * 4.send the request to backend to fetch list of admin data 
 */
...
export async function GET() {
    ...
    console.log("\nAdminDashboard-Manage Accounts get-admin-list> : Nextjs API has Called");
    ...
    if (cookieStore.has('email')) {
        ...
        console.log("admindashboard--->manage accounts--->admin email--> email :" + JSON.stringify(email));
    ...
    console.log("getAdminDetailsList  > email >>>: " + emailValueString);
    ...
    // **************** 4. Get sales summery  *******************
    // *************************************************************
    console.log("getAdminDetalisList--> Nextjs API has Called");
    try{

        const response = await fetch(`http://localhost:8080/api/v1/auth/get-all-admin-list?adminEmail=${emailValueString}`, {
           // default it is GET
            headers: {
        ...
        console.log("get-all-admin-list--> request Sent");
        if (response.ok) {
            ...
            console.log("all admin details fetched successfully:", data);
            return new Response(
                JSON.stringify(data), 
                {
                status: 200,
            ...
            console.warn(response.status + " >> Error from admin details fetching >> " + responseBodyText)
            return new Response(
