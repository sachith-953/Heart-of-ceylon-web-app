...
export async function POST(request: Request) {
  ...
  console.log("\nGet Logs  >: Nextjs API has Called");
  ...
  if (cookieStore.has("email")) {
    ...
    console.log("Get Logs   > email :" + JSON.stringify(email));
    //output :
  ...
  console.log("Get Logs  > email >>>: " + emailValueString);
  ...
  // *************************************************************
  // ********* 4. GetTopSellingProductForAdminDashboard. ******
  // *************************************************************
  console.log("Admindashboard -->Get Logs . Nextjs API has Called");
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/get-logs-for-admin-dashboard?adminEmail=${emailValueString}&pageNumber=${requestedPageNo}`,
      {
        headers: {
    ...
    console.log("get-logs-for-admin-dashboard -----> request Sent");
    if (response.ok) {
      ...
      console.log(
        "Admindashboard --> get-logs-for-admin-dashboard--> data fetched successfully:",
        data
      );
      ...
      console.warn(
        response.status +
          " >> Admindashboard -->get-logs-for-admin-dashboard--> details fetching >> " +
          responseBodyText
      );
