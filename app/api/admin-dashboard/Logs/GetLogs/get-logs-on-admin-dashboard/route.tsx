...
export async function POST(request: Request) {
    ...
    try {
        ...
        const response = await fetch(
            `http://localhost:8080/api/v1/auth/get-logs-for-admin-dashboard?adminEmail=${emailValueString}&pageNumber=${requestedPageNo}`, {
                headers: {
    ...
    if (response.ok) {
        ...
        console.log("Admindashboard --> get-logs-for-admin-dashboard--> data fetched successfully:", data);
        return new Response(
            JSON.stringify(data), 
            {
            status: 200,
        ...
        console.warn(response.status + " >> Admindashboard -->get-logs-for-admin-dashboard--> details fetching >> " + responseBodyText)
        return new Response(
