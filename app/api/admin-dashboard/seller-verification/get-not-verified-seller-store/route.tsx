...
export async function POST(request: Request) {
    ...
    // *************************************************************
    // **************** 4. Get not verified seller's store  *******************
    // *************************************************************

    console.log("getNotVerifiedSellerStore Nextjs API has Called");
    try{

        const requestBody = await request.json();
        const sellerId = requestBody.sellerId;

        const response = await fetch(`${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/get-to-be-verified-seller-store-details?adminEmail=${emailValueString}&sellerId=${sellerId}`, {
           // default it is GET
            headers: {
    ...
    if (response.ok) {
        ...
        console.warn(response.status + " >> Error from getNotVerifiedSellerStore fetching >> " + responseBodyText)
        return new Response(
