...
export async function POST(request: Request) {
    ...
    console.log("\nGet reviews and ratings: Nextjs API has Called");
    ...
    if (cookieStore.has('email')) {
        ...
        console.log("AdminDashboard > product management -->Get reviews and ratings--> popup> email :" + JSON.stringify(email));
        //output : 
    ...
    console.log("AdminDashboard > get reviews and ratings-->popup > email >>>: " + emailValueString);
    ...
    console.log("get reviews and ratings --> Nextjs API has Called");

    try{
        const response = await fetch(`http://localhost:8080/api/v1/auth/get-reviews-for-a-product?adminEmail=${emailValueString}&productID=${productID}`, {
           // default it is GET
            headers: {
        ...
        console.log("Get reviews and ratings for a product --> request Sent to backend");
        if (response.ok) {
            ...
            console.log("reviews and ratings --> fetched successfully:", data);
            return new Response(
                JSON.stringify(data), 
                {
                status: 200,
            ...
            console.warn(response.status + " >> Error from---> fetching reviews and ratings >> " + responseBodyText)
            return new Response(
