...
export async function POST(request: Request) {
  ...
  console.log(
    "AdminDashboard > search products--> search an order: Nextjs API has Called"
  );
  ...
  if (cookieStore.has("email")) {
    ...
    console.log(
      "AdminDashboard > search products--> search an order > email :" +
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/get-products-by-search-from-admin-dashbord?adminEmail=${emailValueString}&searchWord=${searchKey}&pageNumber=${requestedPageNo}`,
      {
        // default it is GET
        headers: {
