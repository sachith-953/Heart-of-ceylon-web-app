...
export async function POST(request: Request) {
  ...
  console.log(
    "AdminDashboard > search to-be verified products--> search an order: Nextjs API has Called"
  );
  ...
  if (cookieStore.has("email")) {
    ...
    console.log(
      "AdminDashboard > search to-be verified products--> search an order > email :" +
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/get-products-by-search-to-verify?adminEmail=${emailValueString}&searchWord=${searchKey}&pageNumber=${requestedPageNo}`,
      {
        // default it is GET
        headers: {
    ...
    if (response.ok) {
      ...
      console.log(
        "all order details by search fetched by search successfully:",
        data
      );
