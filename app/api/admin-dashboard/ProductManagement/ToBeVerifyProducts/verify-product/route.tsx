...
export async function POST(request: Request) {
  ...
  console.log(
    "\nGetProductsToVerifyForAdminDashboard >: Nextjs API has Called"
  );
  ...
  if (cookieStore.has("email")) {
    ...
    console.log("Get Products To Verify  > email :" + JSON.stringify(email));
    //output :
  ...
  console.log("Get Products To Verify   > email >>>: " + emailValueString);
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/refresh-token`,
      {
        method: "POST",
        headers: {
  ...
  // *************************************************************
  // ********* 4. Get Products To Verify in admin dashboard-->product management ******
  // *************************************************************
  console.log(
    "Admindashboard -->Products To Verify -admin-dashboard. Nextjs API has Called"
  );
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/get-products-top-verify-admin-dashboard?adminEmail=${emailValueString}&pageNumber=${requestedPageNo}`,
      {
        headers: {
    ...
    console.log("Products To Verify -admin-dashboard -----> request Sent");
    if (response.ok) {
      ...
      console.log(
        "Admindashboard -->Products To Verify -admin-dashboard--> data fetched successfully:",
        data
      );
      ...
      console.warn(
        response.status +
          " >> Admindashboard --> Products To Verify -admin-dashboard--> details fetching >> " +
          responseBodyText
      );
