...
export async function POST(request: Request) {
    ...
    console.log(" update product note --> search an order: Nextjs API has Called");
    ...
    if (cookieStore.has('email')) {
        ...
        console.log("update product note--> search an order > email :" + JSON.stringify(email));
        //output : 
    ...
    try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/update-product-note-by-admin?adminEmail=${emailValueString}&productID=${productID}&newNote=${newNote}`, {
            method: 'PUT',
            headers: {
