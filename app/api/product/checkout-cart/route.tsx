...
export async function POST(request: Request) {
  ...
  console.log("\ncheckout cart: Nextjs API has Called");
  ...
  try {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/update-cart-when-checkout?email=${emailValueString}`,
      {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({
          ...
          cartProducts: requestBody.cartProducts
        })
      }
    );
    console.log("checkout cart--> request Sent to backend");
    if (response.ok) {
      const data = await response.text(); // .json() since backend service class return DTO list
      // if the backend return a string this should response.text()
      console.log("checkout cart--> fetched successfully:", data);
