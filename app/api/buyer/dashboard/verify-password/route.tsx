...
/**
 * this API use to verify password form dashboard
...
export async function POST(request: Request) {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:8080/api/v1/auth/verify-password?email=${emailValueString}`, {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            const resData = {message: responseBodyText};
            return new Response(JSON.stringify(resData),{status: 401});
        }
    }
    catch (error) {
        console.log(error)
