...
/**
 * this API use to change password form buyer dashboard
 ...
 * 4.send the request to backend to change password
 */
...
export async function POST(request: Request) {
    ...
    console.log("FORM RECIEVED TO THE NEXT JS ENDPOINT > change Password")
    ...
    try {
        ...
        const response = await fetch(`http://localhost:8080/api/v1/auth/update-new-password?email=${emailValueString}`, {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            const responseBodyText = await response.text();
            const resData = {message: responseBodyText};
            console.warn(response.status + " >> Error from change password >> " + responseBodyText)
            return new Response(JSON.stringify(resData), { status: 406 });
        }
    }
    catch (error) {
        console.log(error)
