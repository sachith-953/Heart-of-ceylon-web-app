...
export async function POST(request: Request) {
  ...
  console.log("seller-sign-up > FORM RECIEVED TO THE NEXT JS ENDPOINT")

  try {
    const userData = {
      storeName: formData.get('storeName'),
      sellerEmail: formData.get('sellerEmail'),
      sellerPassword: formData.get('sellerPassword'),
    };

    // const userName = formData.get('uname');
    console.log(JSON.stringify(userData));

    const response = await fetch(`http://localhost:8080/api/v1/sign-up/seller`, {
      method: 'POST',
      headers: {
