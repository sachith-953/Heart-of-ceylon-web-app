...
/**
 * this API use to change product stocks in seller dashboard
...
export async function POST(request: Request) {
  ...
  console.log("update-product-stocks Nextjs API has Called");
  try {
    //get the productId here
    // Get the request body containing productId
    const requestBody = await request.json();
    const { productId, newStockAmount } = requestBody;

    const url = new URL(`${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/update-stocks-amount`);
    url.searchParams.append('sellerEmail', emailValueString);
    url.searchParams.append('productId', productId.toString());
    url.searchParams.append('newStockAmount', newStockAmount.toString())
    ...
    if (!response.ok) {
        ...
        return new Response(
          JSON.stringify({
            error: {
              message: errorText || "Failed to update product stocks",
            },
          }),
      ...
      console.error("Error in update-product-stocks:", error);
      return new Response(
        JSON.stringify({
          error: {
