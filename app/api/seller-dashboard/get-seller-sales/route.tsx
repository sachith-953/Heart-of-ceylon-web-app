...
/**
 * this API use to get seller sales for my sales in seller dashboard
 ...
 * 4.send the request to backend to fetch seller sales for sellar dashboard >> My sales
 */
...
export async function GET() {
    ...
    console.log("getSellerSales Nextjs API has Called");
    try{

        const response = await fetch(`http://localhost:8080/api/v1/auth/get-seller-sales-details?sellerEmail=${emailValueString}`, {
           // default it is GET
            headers: {
    ...
    console.log("getSellerSales request Sent");
    if (response.ok) {
        ...
        console.log("seller sales fetched successfully:", data);
        return new Response(
            JSON.stringify(data), 
            {
            status: 200,
        ...
        console.warn(response.status + " >> Error from seller sales fetching >> " + responseBodyText)
        return new Response(
