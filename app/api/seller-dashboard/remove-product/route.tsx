...
/**
 * this API use to remove product in seller dashboard
...
export async function POST(request: Request) {
  ...
  console.log("remove-product Nextjs API has Called");
  try {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SPRING_BOOT_SERVER_URL}/api/v1/auth/delete-product-by-seller?sellerEmail=${emailValueString}&productId=${productId}`,
      {
        method: "PUT",
        headers: {
    ...
    if (response.ok) {
      ...
      console.warn(
        response.status +
          " >> Error from update-seller-private details >> " +
          responseBodyText
      );
