...
const UpdateProductNote: React.FC<ChildProps> = ({ note, productId, updateParentNote, }) => {
    ...
    const handleNoteUpdate = async () => {
        ...
        try {
            const res = await fetch(`${BASE_URL}/api/admin-dashboard/ProductManagement/update-product-note`, {
                method: 'POST',
                headers: {
        ...
        <>
            <div className="bg-white">
                ...
                <button
                    ...
                    className={`bg-blue-600 hover:bg-blue-800 text-white font-bold py-2 rounded w-1/5 px-4 hover:text-black flex flex-row justify-center ${loading ? 'cursor-not-allowed' : ''
                        }`}
                    disabled={loading}
                >
                    {loading ? (
